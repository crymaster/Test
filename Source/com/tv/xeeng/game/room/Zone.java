...
import com.tv.xeeng.debug.DebugUtils;
...
//		synchronized (this.mRooms) {
        System.out.println(" -----> create room: " + des + " | " + id + " | "+ phong);
...
//						break;
    ...
    public ConcurrentHashMap<Integer, Phong> getMPhongs() {
        return mPhongs;
    }
    ...
    public void initPhong() throws SQLException, DBException {
        ...
        mLog.debug("[initPhong] zoneId:" + mZoneId);
        for (NRoomEntity r : rs) {
            ...
            mLog.debug("[initPhong] " + id + " " + mZoneId + " " + r.getLv() );
        }
    }
    ...
    public Phong xeGetPhong(int levelID) {
        mLog.debug("DEBUGUTILS req.levelID:" + levelID);
        for (Phong p : mPhongs.values()) {
            if (p.level == levelID) {
                return p;
            }
