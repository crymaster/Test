...
public class InviteBusiness extends AbstractBusiness {
	...
	private static final Logger mLog = LoggerContext.getLoggerFactory()
			.getLogger(InviteBusiness.class);

	public int handleMessage(ISession aSession, IRequestMessage aReqMsg,
			IResponsePackage aResPkg) {
		int rtn = PROCESS_FAILURE;
		MessageFactory msgFactory = aSession.getMessageFactory();
		InviteResponse resInvite = (InviteResponse) msgFactory
		...
		try {
			...
			if (buddySession != null) {
				...
				// if ( buddySession.getCurrentZone() > 0 &&
				// buddySession.getCurrentZone() != aSession.getCurrentZone()) {
				if ((buddySession.getCurrentZone() != aSession.getCurrentZone())
						&& (buddySession.getCurrentZone() > 0)) {
					resInvite.setFailure(ResponseCode.FAILURE,
					...
					resInvite.setFailure(ResponseCode.FAILURE,
							"Bạn ý đang chơi ở room khác mất rồi");
					aSession.write(resInvite);
				} else {
					...
					if (newUser != null) {
						...
						resInvite.setSuccess(ResponseCode.SUCCESS, sourceID,
								roomID, room.getName(), aSession.getUserName(),
								table.getMinBet(), table.getLevel());
						resInvite.timeout = ZoneID.getTimeout(aSession
								.getCurrentZone());
