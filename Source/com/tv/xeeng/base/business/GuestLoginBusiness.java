...
//        mLog.debug("[Guest LOGIN]: Catch");
        ...
        try {
            if (DBCache.isMaintenance) {
                throw new Exception("Hệ thống đang bảo trì. Bạn vui lòng quay trở lại sau nhé!");
            }
            ...
            if (rqLogin.getOsName() != null) {
                if (rqLogin.getOsName().toUpperCase().contains("ANDROID")) {
                    mLog.debug("android device");
                    aSession.setDeviceType(AIOConstants.ANDROID_DEVICE);
                } else if (rqLogin.getOsName().toUpperCase().contains("IPHONE") || rqLogin.getOsName().toUpperCase().contains("IPOD") || rqLogin.mobileVersion.toUpperCase().contains("IPAD")) {
                ...
                device += rqLogin.getOsName() + ";" + rqLogin.getOsVersion() + ";" + rqLogin.getOsMAC();
            }

            resLogin.isMobile = true;
//            user = userDb.guestLogin(rqLogin.deviceUId, rqLogin.partnerId, rqLogin.refCode, rqLogin.mobileVersion);
            user = userDb.newGuestLogin(rqLogin.deviceUId, rqLogin.partnerId, rqLogin.refCode, device, rqLogin.regTime);
...
//                                    resLogin.setLastMatch(curMatchID);
                ...
                if (latestVersion != null && !rqLogin.mobileVersion.equals(latestVersion.desc)
                        && aSession.getDeviceType() == AIOConstants.ANDROID_DEVICE && rqLogin.partnerId == 96) {
//                if (latestVersion != null && !rqLogin.mobileVersion.equals(latestVersion.desc) && aSession.getDeviceType() == AIOConstants.IPHONE_DEVICE) { 
                ...
                // New advertising by thangtd
                addNewAvds(aSession, msgFactory);
            }
    ...
    private void addNewAvds(ISession aSession, MessageFactory msgFactory) {
        List<XEAdsEntity> allAds = XEDataUtils.getAllAds();
        if (allAds != null) {
            XEGetAllAdsResponse advRes = (XEGetAllAdsResponse) msgFactory.getResponseMessage(MessagesID.ADVERTISING_NEW);
            advRes.session = aSession;
            advRes.setSuccess(ResponseCode.SUCCESS, allAds);
