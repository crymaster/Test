...
public class XEExchangeXeengToGoldBusiness extends AbstractBusiness {
	...
	public int handleMessage(ISession aSession, IRequestMessage aReqMsg, IResponsePackage aResPkg) throws ServerException {
		...
		if (user == null || user.mUsername == null) {
			...
			boughtGold = numOfGold;
			vipBonus = XEDataUtils.getBonusPercent(rqExchange.getUserId());
			System.out.println("XEExchangeXeengToGold----userid: " + rqExchange.getUserId() + " bonus: " + bonus);
			numOfGold += numOfGold * (bonus + vipBonus) / 100;
			...
			if (!validAmount) {
				...
				if (user.xeeng < numOfXeeng) {
					...
					try {
						success = XEDataUtils.updateUserMoney(rqExchange.getUserId(), user.xeeng, user.money, boughtGold);
					} catch (SQLException ex) {
						mLog.error(ex.getMessage());
					}

					if (success) {
						String msg = "Bạn đã đổi thành công " + Utils.convertValue(numOfGold) + " Gold.";
						if (bonus > 0 && vipBonus > 0) {
								msg = msg + " (Tặng thêm " + bonus + "% từ sự kiện Giờ Vàng Khuyến Mại và khuyến mại VIP).";
						} else {
							if (bonus > 0) {
								msg = msg + " (Tặng thêm " + bonus + "% từ sự kiện Giờ Vàng Khuyến Mại).";
							}
							if (vipBonus > 0) {
								msg = msg + " (Tặng thêm " + vipBonus + "% từ khuyến mại VIP).";
							}
						}

						resExchange.mCode = ResponseCode.SUCCESS;
						resExchange.setEncodedData(msg);
