...
public class ChargingBusiness extends AbstractBusiness {
	...
	private static final int[] SPECIAL_PARTNER_IDS = {92, 93};
	...
	public int handleMessage(ISession aSession, IRequestMessage aReqMsg, IResponsePackage aResPkg) {
		...
		try {
			...
			int convertedPartnerId = Integer.parseInt(BlahBlahUtil.convertParnterId(partnerId));
			ArrayList<ChargingInfo> temp = db.getChargings(convertedPartnerId, entity.cellPhone);

//            if (aSession.getByteProtocol() >= AIOConstants.PROTOCOL_REFACTOR_BIENG) {
			StringBuilder sb = new StringBuilder();

			// 92, 93 indicates Appota version. Only shows appota's supported card then.
			List<ChargingInfo> cardInfo = db.getLstCardInfo(Arrays.binarySearch(SPECIAL_PARTNER_IDS, partnerId) > -1 ? partnerId : -1);
