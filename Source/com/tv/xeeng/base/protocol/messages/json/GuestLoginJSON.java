					...
					if (arrValues.length > 5) {
						...
						if (deviceParts.length >= 3) {
							...
							if (deviceParts.length >= 4) {
								String extraStr = deviceParts[3];
								String[] extraParts = extraStr.split("&");
								if (extraParts.length >= 2) {
									String[] publisherParts = extraParts[1].split("=");
									if (publisherParts.length == 2) {
										try {
											login.partnerId = Integer.parseInt(publisherParts[1]);
										} catch (NumberFormatException e) {
											mLog.debug("[GuestLoginJSON] Get partnerId from Google Play link. Malformed in publisherId value: publisherId(" + publisherParts[1] + ") login.device(" + login.getOsName() + ";" + login.getOsVersion() + ";" + login.getOsMAC() + ";" + extraStr + ")");
										}
									}
