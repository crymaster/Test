...
public class TurnJSON implements IMessageProtocol {
    ...
    public boolean decode(Object aEncodedObj, IRequestMessage aDecodingObj) throws ServerException {
        try {
            ...
            if (jsonData.has("v")) {
                System.out.println(" match turn json data: " + jsonData.getString("v"));
                String[] arrV = jsonData.getString("v").split(AIOConstants.SEPERATOR_BYTE_1);
//                System.out.println(" arrV[] match turn: " + arrV.length);
...
//                }
                ...
                } else if (zoneId == ZoneID.XI_TO) {
                    matchTurn.xitoAction = Integer.parseInt(arrV[2]);
                    if (arrV.length >= 4) {
                        matchTurn.actionValue = arrV[3];
                    }
                } else {
                    //tien len || sam
                    if (arrV.length > 2) {
