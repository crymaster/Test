 ...
 * @author thangtd
 */
public class XEGetRouletteGuideJSON implements IMessageProtocol {

    private static final Logger mLog = LoggerContext.getLoggerFactory().getLogger(XEGetRouletteGuideJSON.class);
    ...
    public Object encode(IResponseMessage paramIResponseMessage) throws ServerException {
        try {
            ...
            XEGetRouletteGuideResponse response = (XEGetRouletteGuideResponse) paramIResponseMessage;
            ...
            if (response.mCode == ResponseCode.FAILURE) {
                ...
                if (size != 0) {
                    ...
                    for (int i = 0; i < size; i++) {
                        XERouletteItemEntity item = response.itemsList.get(i);
                        sb1.append(item.getRound()).append(AIOConstants.SEPERATOR_BYTE_1);
                        sb1.append(item.getLastItem()).append(AIOConstants.SEPERATOR_BYTE_1);
                        sb1.append(item.getNextItem()).append(AIOConstants.SEPERATOR_BYTE_1);
                        sb1.append(item.getPrice()).append(AIOConstants.SEPERATOR_BYTE_2);
                    }
